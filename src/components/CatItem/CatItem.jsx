import s from './CatItem.module.css'

function CatItem({ item, changeFav}) {
  const changeStat = (id) =>{
    changeFav(id)
  }
  return (
    <div className={! localStorage.getItem(item.id) ? s.divtest : s.divtest + ' ' + s._active}>
      <img src={item.url} alt="" />
      <button className={s.svg} onClick={() => changeStat(item.id)}>
